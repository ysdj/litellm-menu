...
1. **Ordered protocol fallback within one deployment** — each deployment uses its `supported_upstream_url_surfaces` list exactly as configured, for example `openai/responses` → `anthropic` → `openai/chat`.
...
- `supported_upstream_url_surfaces` — ordered API surfaces (`openai/responses`, `openai/chat`, `anthropic`); the first item is primary.
...
The model editor has one **Probe & Recommend** action. For text models it checks Responses, Anthropic Messages, and Chat Completions serially, labels each result as available, unavailable, or uncertain, and defaults to only the best available protocol; extra checked protocols and their order remain user-controlled. Known standalone image models probe only `/v1/images/generations`; an unconfigured new model is classified as an image model only when all three text protocols are definitively unavailable and the image probe succeeds.
...
1. **部署内有序协议回退** — 每个部署严格按 `supported_upstream_url_surfaces` 配置尝试，例如 `openai/responses` → `anthropic` → `openai/chat`。
...
- `supported_upstream_url_surfaces` — 有序 API 接口列表（`openai/responses`、`openai/chat`、`anthropic`），第一项是主协议。
- `upstream_url_surface` — 自动生成的第一项镜像。
- `supports_responses_web_search` / `supports_web_search` — 网页搜索能力。

模型编辑器只保留一个 **Probe & Recommend** 操作。文本模型会依次探测 Responses、Anthropic Messages 和 Chat Completions，并将每项标为可用、不可用或不确定；默认只勾选最佳可用协议，额外协议及其顺序由用户决定。已知的独立图片模型只探测 `/v1/images/generations`；尚未配置的新模型只有在三个文本协议都明确不可用且图片探针成功时，才会被识别为图片模型。
