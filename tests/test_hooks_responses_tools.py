...
class HookResponsesToolBridgeTests(HookTestCase):
    ...
    def test_responses_input_repairs_function_tool_item_id_prefixes(self) -> None:
        hooks, _ = load_hook_module()
        request_input = [
            {
                "type": "function_call",
                "id": "item_generated_call",
                "call_id": "call_exec",
                "name": "exec_command",
                "arguments": '{"cmd":"pwd"}',
            },
        ...
        self.assertEqual(normalized[0]["id"], "fc_item_generated_call")
        self.assertEqual(normalized[1]["id"], "fco_item_generated_output")
        self.assertEqual(normalized[2]["id"], "fc_valid")
        self.assertEqual(normalized[0]["call_id"], "call_exec")
        self.assertEqual(normalized[1]["call_id"], "call_exec")
        self.assertEqual(request_input[0]["id"], "item_generated_call")
        self.assertEqual(
            stats,
            {"changed": True, "normalized_item_ids": 2},
        )
