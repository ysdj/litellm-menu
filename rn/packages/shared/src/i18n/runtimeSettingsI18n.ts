...
const zh: Record<string, RuntimeCopy> = {
  ...
  LITELLM_MENU_WEB_SEARCH_READ_RESULTS: { label: "可读页面数", help: "通过 Jina Reader 展开以获取更强摘要的前几条搜索结果数。设为 0 可关闭页面展开。" },
  LITELLM_MENU_WEB_SEARCH_READ_CHARS: { label: "可读页面字符数", help: "每个展开结果保留的最大 Jina Reader 摘录字符数。" },
  LITELLM_MENU_WEB_SEARCH_DDGS_BACKEND: { label: "DDGS 后端", help: "DDGS 后端列表。使用 auto 进行 DDGS 聚合；也可用逗号或空格分隔 brave、bing、duckduckgo 等后端，手动聚合去重。" },
  ...
  LITELLM_MENU_BALANCE_REFRESH_MINUTES: { label: "余额刷新", help: "“供应商与模型”窗口打开时在后台刷新模型余额。设为 0 可关闭定时刷新；模型探测仍会刷新一次。" },
  LITELLM_BROWSER_BILLING: { label: "浏览器计费回退", help: "直接计费端点失败后，允许请求已打开的同源 Chrome 页面。不会读取 Chrome 配置或 Cookie 文件、打开或切换标签页，也不会启动 Chrome。" },
